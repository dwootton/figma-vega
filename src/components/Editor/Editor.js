...
const Editor = ({ view, onBack, onEditView }) => {
    ...
    React.useEffect(() => {
        ...
        if (view.annotationSpec && view.annotationSpec.marks && view.annotationSpec.marks.length > 0) {
            // merge specs
            const mergedSpec = mergeVisualizationAndAnnotationSpec(view.visualizationSpec, view.annotationSpec);
            ...
            store.addNotification({
                ...
                message: (React.createElement(Text, { addToEditor: () => {
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: "flex", "width": "100%", "border-bottom": "1px solid #4F4F4F" } },
            " ",
            React.createElement("div", { style: { cursor: "pointer" } },
                React.createElement(Icon, { onClick: onBack, name: "back" }),
                " "),
            React.createElement("div", null,
                " ",
                React.createElement(Title, { size: "xlarge", className: 'viewTitle', styles: { width: "350px", margin: '0px 0px 0px 24px' } }, viewName))),
        React.createElement("div", { style: { display: "flex" } },
            React.createElement(VegaSpec, { currentSpec: spec, visualizationNodeId: visualizationNodeId, annotationSpec: annotationSpec, onChange: setUnsavedSet }),
            React.createElement(Visualization, { errorMessage: message })),
        React.createElement("div", { style: { display: "flex", margin: "8px" } },
            React.createElement(Button, { className: 'actionButton left', id: 'preview', onClick: onPreview, isSecondary: true }, "Preview Visualization"),
            React.createElement(Button, { className: 'actionButton right', id: 'fetch', disabled: !isSavedToDocument, onClick: onFetchSVG, isSecondary: true }, "Get Annotations"),
            React.createElement(Button, { className: 'actionButton', id: 'create', onClick: () => onCreate(view.viewId) }, isSavedToDocument ? "Update Document" : "Save to Document"))));
};
const Text = ({ addToEditor }) => {
    return (React.createElement("div", null,
        "Updated Vega specification has been copied to your clipboard.",
        " ",
        React.createElement("span", { style: { textDecoration: "underline" }, onClick: addToEditor }, "Add to Vega Editor?")));
};
...
function mergeVisualizationAndAnnotationSpec(visualizationSpec, annotationSpec) {
    ...
    if (specCopy.marks) {
        for (const mark of annotationSpec.marks) {
            const currentMarkIndex = specCopy.marks.findIndex((existingMark) => JSON.stringify(existingMark) === JSON.stringify(mark));
...
const VegaSpec = ({ currentSpec, annotationSpec, visualizationNodeId, onChange }) => {
    ...
    return (React.createElement("div", null,
        React.createElement(ResizableBox, { width: 300, height: 550, axis: "x", handleSize: [8, 8], resizeHandles: ["e"] },
            React.createElement(ControlledEditor, { language: 'json', editorDidMount: handleEditorDidMount, options: {
                    formatOnPaste: true,
                ...
                }, value: currentSpec ? JSON.stringify(currentSpec, undefined, 2) : "Paste Vega Spec here", onChange: (ev, value) => {
                    onChange(value);
                } }))));
};
const Visualization = ({ errorMessage }) => {
    return (React.createElement("div", { style: {
        ...
        errorMessage && (React.createElement("div", { style: { color: "#D8000C", backgroundColor: "#FFBABA", border: 0, padding: "10px" } },
            errorMessage,
