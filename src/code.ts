...
figma.ui.onmessage = (msg) => {
  ...
  if (msg.type === "fetch") {
    for (const sceneNode of figma.currentPage.selection) {
      ...
      const node = sceneNode as VectorNode;
      ...
      const newPaths = paths.map((path) =>
        getTransformedPathDStr(path, pathTranslX, pathTranslY, scale)
      );
...
function calculateXYFromNode(node) {
  ...
  const realX =
    x + (1 / 2) * width * Math.cos(rot) - -1 * (1 / 2) * height * Math.sin(rot) - (1 / 2) * width;
  const realY =
