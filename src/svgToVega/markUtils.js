...
export function generateRectSpec(element) {
    const rectSpec = { type: "rect", encode: { enter: {} } };
    const rectProperties = convertProperties(element.properties);
    rectSpec.encode.enter = rectProperties;
    return rectSpec;
}
export function generatePathSpec(element) {
    const pathSpec = { type: "path", encode: { enter: { path: "" } } };
    const pathProperties = convertProperties(element.properties);
    pathSpec.encode.enter = pathProperties;
    return pathSpec;
}
export function generateImageSpec(element) {
    let imageSpec = { type: "image", encode: { enter: {} } };
    const imageProperties = convertProperties(element.properties);
...
export function calculateNormalizedBoundingBox(gradientElement, boundingElement) {
    ...
    const boundingX = extractProperty("x", boundingElement), boundingY = extractProperty("y", boundingElement), boundingWidth = extractProperty("width", boundingElement), boundingHeight = extractProperty("height", boundingElement);
    const elementX1 = extractProperty("x1", gradientElement), elementX2 = extractProperty("x2", gradientElement), elementY1 = extractProperty("y1", gradientElement), elementY2 = extractProperty("y2", gradientElement);
...
export function extractStops(gradientElement) {
    ...
    for (const child of gradientElement.children) {
        if (child.tagName === "stop") {
            // extract rgba color:
            let colorInfo = get(child.properties["stop-color"]), alpha = 1;
...
function convertCircleToPath(element) {
    let cx = extractProperty("cx", element), cy = extractProperty("cy", element), rx = extractProperty("rx", element), ry = extractProperty("ry", element), r = extractProperty("r", element);
    if (r) {
        rx = r;
        ry = r;
    }
...
export function offsetProperties(properties, offsets) {
    ...
    if (properties) {
        let xOffset = ((_a = properties["x"]) === null || _a === void 0 ? void 0 : _a.value) ? properties["x"].value : 0;
        let yOffset = ((_b = properties["y"]) === null || _b === void 0 ? void 0 : _b.value) ? properties["y"].value : 0;
